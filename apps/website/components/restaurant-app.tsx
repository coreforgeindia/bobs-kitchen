...
import { supabase, generateUpiUri, generateAppUpiUris, reserveOrderId, PAYMENT_TIMEOUT_SECONDS, PAYMENT_POLL_INTERVAL_MS, UPI_CONFIG } from '@/lib/supabase'
...
function menuItemFromRow(row: any): MenuItem {
  const localMenuItem = menuItems.find((item) => item.id === row.id || item.name.toLowerCase() === String(row.name || '').toLowerCase())
  return {
    id: row.id,
    name: row.name,
...
function useLiveMenuItems() {
  ...
  useEffect(() => {
    ...
    const loadMenu = async () => {
      ...
      if (!error && data?.length && mounted) setLiveMenuItems(data.map(menuItemFromRow))
    }

    loadMenu()
    const channel = supabase
      .channel('website-menu-catalog')
...
function orderFromSupabaseRow(row: any): Order {
  return {
    id: row.order_id || row.id,
    items: Array.isArray(row.items) ? row.items.map((item: any, index: number) => {
      const menuItem = menuItems.find((candidate) => candidate.id === item.id || candidate.name.toLowerCase() === String(item.name || '').toLowerCase())
      return {
        ...(menuItem || { id: item.id || `item-${index}`, name: item.name || 'Menu item', category: 'Menu', rating: 0, calories: 0, time: '', available: true }),
        id: item.id || menuItem?.id || `item-${index}`,
        name: item.name || menuItem?.name || 'Menu item',
        price: Number(item.price || menuItem?.price || 0),
        quantity: Number(item.qty || item.quantity || 1),
        veg: typeof item.veg === 'boolean' ? item.veg : menuItem?.veg ?? !/(chicken|meat|egg|tikka|jalfrezi)/i.test(String(item.name || '')),
      }
    }) : [],
...
function AddressSelectionModal({
  ...
  onSelectAddress: (address: string, distanceKm: number, lat?: number, lng?: number) => void
}) {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          onSelectAddress(readable, dist, lat, lng)
          ...
          onSelectAddress(fallback, dist, lat, lng)
        } finally {
          setDetecting(false)
          onClose()
        }
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    setCurrentDist(d)
                    onSelectAddress(r.display_name, d, parseFloat(r.lat), parseFloat(r.lon))
                    onClose()
    ...
    <header className="sticky top-0 z-40 border-b border-border/80 bg-white/95 shadow-xs backdrop-blur-md transition-all text-foreground font-sans">
      <div className="mx-auto flex h-14 sm:h-16 md:h-20 max-w-7xl items-center justify-between px-3 sm:px-4 md:px-6 lg:px-8 gap-2 sm:gap-3 md:gap-4 lg:gap-6">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 shrink-0">
          ...
          <button onClick={onTrackOrders} className="hidden sm:flex items-center rounded-full border border-orange-500/30 bg-orange-500/10 px-3 py-2 text-[10px] font-black text-orange-600 hover:bg-orange-500 hover:text-white">
            Track Order
          </button>
          <button 
            onClick={onCart} 
...
function PhoneOrderLookup({ open, onClose, onTrack }: { open: boolean; onClose: () => void; onTrack: (orderId: string, order: any) => void }) {
  ...
  const findOrders = async (event: React.FormEvent) => {
    event.preventDefault()
    const normalizedPhone = phone.replace(/\D/g, '').slice(-10)
    if (normalizedPhone.length < 10) {
      setMessage('Enter the phone number used for your order.')
      return
    }
    setLoading(true)
    setMessage('')
    const { data, error } = await supabase.from('orders').select('*').in('customer_phone', [normalizedPhone, `+91${normalizedPhone}`, `91${normalizedPhone}`, `+91 ${normalizedPhone}`]).order('created_at', { ascending: false })
    setLoading(false)
    if (error) setMessage('Unable to fetch orders right now.')
    else if (!data?.length) setMessage('No orders found for this phone number.')
    else setOrders(data)
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-3xl bg-white p-5 text-slate-900 shadow-2xl">
        <div className="flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-widest text-orange-600">Guest order access</p><h2 className="text-xl font-black">Track by phone</h2></div><button onClick={onClose} className="rounded-full p-2 text-slate-500 hover:bg-slate-100"><X size={18} /></button></div>
        <form onSubmit={findOrders} className="mt-4 flex gap-2"><input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="10-digit phone number" inputMode="tel" className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm font-bold outline-none focus:border-orange-500" /><button className="rounded-xl bg-orange-500 px-4 text-xs font-black text-white">{loading ? 'Finding...' : 'Find Orders'}</button></form>
        {message && <p className="mt-3 text-xs font-bold text-slate-500">{message}</p>}
        <div className="mt-4 space-y-2">{orders.map((order) => <button key={order.order_id} onClick={() => { onClose(); onTrack(order.order_id, order) }} className="flex w-full items-center justify-between rounded-xl border border-slate-200 bg-slate-50 p-3 text-left hover:border-orange-400"><span><strong className="font-mono text-sm">#{order.order_id}</strong><span className="ml-2 text-xs font-bold text-slate-600">{order.order_mode} · {order.status}</span></span><span className="text-xs font-black text-orange-600">Track</span></button>)}</div>
      </div>
    </div>
  )
}
...
function OrderTrackingModal({ 
  ...
  const steps = [
    ...
    { title: 'Order Confirmed', desc: 'Verified and accepted by kitchen staff' },
    { title: 'Preparing Food', desc: 'Flame grilling & packing fresh' },
    { title: order.mode === 'Takeaway' ? 'Ready to Pick Up' : 'Out for Delivery', desc: order.mode === 'Takeaway' ? 'Pick up at Kaveri Layout Outlet' : 'Express rider on the way' },
  ]

  let currentStepIdx = 0
  if (order.status === 'Restaurant Accepted') currentStepIdx = 1
  else if (order.status === 'Preparing' || order.status === 'Packed') currentStepIdx = 2
  else if (order.status === 'Out For Delivery' || order.status === 'Ready for Pickup' || order.status === 'Picked Up' || order.status === 'Delivery Partner Assigned' || order.status === 'Delivered') currentStepIdx = 3
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 font-sans">
      ...
      <motion.div 
        ...
        animate={{ opacity: 1, scale: 1, y: 0 }} 
        exit={{ opacity: 0, scale: 0.95, y: 15 }} 
        className="relative z-10 w-full max-w-md overflow-hidden rounded-3xl border-2 border-orange-200 bg-white text-slate-900 font-outfit shadow-2xl space-y-4 max-h-[92vh] overflow-y-auto p-5 sm:p-6"
      >
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div>
            ...
            <h3 className="text-lg sm:text-xl font-black mt-0.5 text-slate-900">Track Your Feast</h3>
          </div>
        ...
        <div className="rounded-2xl bg-orange-50/60 p-4 border border-orange-200/80 space-y-3.5">
          ...
          <div className="relative ml-2 space-y-3 pl-6">
            <div className="absolute left-[2px] top-2.5 bottom-2.5 w-0.5 bg-orange-400" aria-hidden="true" />
            {steps.map((st, idx) => {
        ...
        <div className="flex gap-2 pt-1 font-outfit">
          <a
  ...
  const handleCompleteAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        login(name.trim(), email.trim(), phone.trim(), address.trim())
...
function HomeView({ setView, onSelectCategory, onOpenPermissions }: { setView: (v: ViewType) => void; onSelectCategory: (cat: string) => void; onOpenPermissions: () => void }) {
  const liveMenuItems = useLiveMenuItems()
  const fusionDishes = liveMenuItems.filter((i) => i.category === 'Fusion Snack Pack')
...
function MenuView({ onBack, initialCategory = 'All' }: { onBack: () => void; initialCategory?: string }) {
  ...
  const itemsForCategory = (catName: string) => {
    return liveMenuItems.filter((dish) => {
  ...
  const totalItemsFound = useMemo(() => {
    return activeCategories.reduce((sum, catName) => sum + itemsForCategory(catName).length, 0)
  }, [activeCategories, liveMenuItems, search, vegOnly, nonVegOnly, bestsellersOnly])
  ...
  return (
    <motion.main initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mx-auto max-w-7xl px-2 sm:px-4 md:px-6 lg:px-8 py-3 sm:py-6 md:py-8 font-outfit">
      ...
      <div className="space-y-3.5 sm:space-y-6">
        {totalItemsFound === 0 ? (
          <div className="text-center py-16 bg-card rounded-3xl border border-border/85 p-6 space-y-3 font-outfit">
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-500/10 text-amber-500">
              <AlertTriangle size={28} />
            </div>
            <h3 className="text-sm font-black text-slate-900 uppercase tracking-wider">No Matching Dishes</h3>
            <p className="text-xs text-slate-500 max-w-xs mx-auto leading-relaxed">
              We couldn&apos;t find any items matching &quot;{search}&quot;. Try adjusting filters or search query.
            </p>
            <button
              onClick={() => { setSearch(''); setVegOnly(false); setNonVegOnly(false); setBestsellersOnly(false); setSelectedCategory('All'); }}
              className="rounded-full bg-orange-500 text-white font-extrabold px-5 py-2 text-xs hover:bg-orange-600 transition-all cursor-pointer shadow-sm"
            >
              Clear All Filters
            </button>
          </div>
...
function SpecialsView({ setView }: { setView: (v: ViewType) => void }) {
  const liveMenuItems = useLiveMenuItems()
  const fusionSnackPackItems = liveMenuItems.filter((i) => i.category === 'Fusion Snack Pack')
  const addToCart = useAppStore((s) => s.addToCart)
...
function Checkout({ 
  ...
  const [paymentMethod, setPaymentMethod] = useState<'qr' | 'intent'>('qr')
  ...
  const upiLinks = useMemo(() => {
    if (!qrOrderId) return null
    return generateAppUpiUris(total, qrOrderId)
  }, [total, qrOrderId])
  ...
  }, [total, user, guestName, guestPhone, guestEmail, activeAddr, orderMode, cart, deliveryCoordinates])
  ...
  // Regenerate QR when checkout step reaches step 3 or total changes
  useEffect(() => {
    if (checkoutStep === 3 && total > 0 && paymentStatus === 'idle') {
      generatePaymentQR()
    }
  }, [checkoutStep, total, paymentStatus])
  ...
  const handleConfirmOrder = async (overrideOrderId?: string, overrideTxnId?: string) => {
    if (cart.length === 0) return toast.error('Your basket is empty!')

    // Validate phone number
    const finalPhone = (user?.phone || guestPhone.trim()).replace(/\D/g, '').slice(-10)
    if (!finalPhone) {
      return toast.error('Please enter a valid Phone Number!')
    }

    // If delivery, validate name
    const finalName = user?.name || guestName.trim()
    const finalEmail = user?.email || guestEmail.trim()
    ...
    const orderIdToSave = overrideOrderId || qrOrderId
    if (!orderIdToSave) return toast.error('Order number is unavailable. Please restart checkout.')

    // Auto-register user locally if not logged in
    ...
    const id = placeOrder({
      ...
      paymentMethod: paymentMethod === 'qr' ? 'UPI QR' : 'UPI Link',
      ...
      <div className="min-h-screen bg-[#F2F4F7] py-8 sm:py-12 px-3 sm:px-4 font-outfit flex flex-col items-center justify-center text-slate-900">
        <div className="max-w-md w-full rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-2xl text-center space-y-4">
          ...
          <div className="rounded-2xl bg-slate-50 p-4 border border-slate-200 text-left text-xs space-y-2 font-outfit">
            ...
            <p className="font-bold text-slate-700 pt-1 border-t border-slate-200">Total Paid: <span className="font-black text-slate-900 text-sm">{formatPrice(placedTotal)}</span></p>
          </div>
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white px-4 py-3 sm:px-8 shadow-xs font-outfit">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-xs font-bold text-slate-600 font-outfit">
            <span className="flex items-center gap-1 text-slate-900 font-black font-mono">
              📞 {user?.phone || guestPhone || 'Active Checkout'}
            </span>
          </div>
        </div>
          </header>
      ...
      <main className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8 pt-6 sm:pt-8 font-outfit">
        <div className="grid gap-6 lg:grid-cols-12 items-start">
          ...
          <div className="lg:col-span-7 space-y-4 font-outfit">
            ...
            {/* MOBILE ORDER SUMMARY DROPDOWN (ONLY SHOWS ON MOBILE < LG) */}
            <div className="lg:hidden rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden font-outfit">
              <button
                type="button"
                onClick={() => setIsSummaryExpanded(!isSummaryExpanded)}
                className="w-full flex items-center justify-between p-4 bg-slate-50 border-b border-slate-100 cursor-pointer"
              >
                <div className="flex items-center gap-2">
                  <span className="text-xs font-black text-slate-500">🛒 ORDER SUMMARY:</span>
                  <span className="font-black text-slate-900 text-sm">{formatPrice(total)}</span>
                </div>
                <div className="flex items-center gap-1 text-xs font-bold text-orange-600">
                  ...
                  <ChevronDown className={`transition-transform duration-200 ${isSummaryExpanded ? 'rotate-180' : ''}`} size={16} />
                </div>
              </button>
              ...
              <AnimatePresence>
                {isSummaryExpanded && (
                  <motion.div 
                    ...
                    className="p-4 border-t border-slate-100 bg-white space-y-4 text-xs overflow-hidden"
                  >
                    {/* Items List */}
                      ...
                      {cart.map((item) => (
                        <div key={item.id} className="flex items-center justify-between">
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <span className="font-bold text-slate-900 truncate">{item.name} x {item.quantity}</span>
                          </div>
                    ...
                    {/* Cost Breakdown */}
                    <div className="pt-2 border-t border-slate-100 space-y-1.5 font-bold text-slate-500">
            ...
            <div className={`rounded-3xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm space-y-3 transition-all ${checkoutStep !== 1 ? 'opacity-85' : 'ring-2 ring-orange-500'}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-900 text-white font-black text-xs font-outfit">
                    <span className="font-outfit text-sm">1</span>
                  </div>
                  <div>
                    <h3 className="font-outfit text-base sm:text-lg font-black text-slate-900">Customer Contact</h3>
                    {checkoutStep !== 1 && (
                ...
                {checkoutStep !== 1 && (
                  <button 
                    ...
                    className="rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700 cursor-pointer shadow-2xs font-outfit"
                  >
                    Change Info
                  </button>
                )}
              ...
              {checkoutStep === 1 && (
                <div className="space-y-4 animate-in fade-in duration-200">
                  {user ? (
                    <div className="rounded-2xl bg-orange-50/50 p-4 border border-orange-200 text-xs font-outfit space-y-2">
                      <div className="flex items-center justify-between">
                        <p className="font-black text-slate-900 text-sm flex items-center gap-1.5">
                          <span>👤</span>
                          <span>{user.name}</span>
                        </p>
                        <span className="text-emerald-700 font-extrabold text-[11px] bg-emerald-100/80 px-2 py-0.5 rounded-md font-outfit">Saved Locally</span>
                      </div>
                      <p className="text-slate-600 flex items-center gap-1.5">
                        <span>📞</span>
                        <span>Phone: <strong className="text-slate-900 font-mono">{user.phone}</strong></span>
                      </p>
                      <div className="pt-2 flex justify-between gap-2">
                        <button 
                          type="button" 
                          onClick={() => logout()} 
                          className="rounded-lg border border-slate-200 bg-white hover:bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700 cursor-pointer"
                        >
                          Clear Saved Profile
                        </button>
                        <button
                          ...
                          className="rounded-lg bg-orange-500 hover:bg-orange-600 text-white font-black px-4 py-1.5 text-xs cursor-pointer"
                        >
                          Continue →
                        </button>
                      </div>
                    ...
                    <div className="space-y-3 pt-1">
                      <div className="grid gap-3 sm:grid-cols-2 text-xs font-outfit">
                        <div>
                      ...
                      <label className="flex items-center gap-2 text-xs text-slate-600 cursor-pointer pt-1">
                        ...
                        />
                        <span>Save my details locally so I don&apos;t have to enter them next time</span>
                      </label>

                      <div className="pt-2 flex justify-end">
                        <button
                          type="button"
                          disabled={!guestPhone.trim() || !guestName.trim()}
                          onClick={() => {
                            if (saveAccount) {
                              login(guestName, '', guestPhone, activeAddr)
                            }
                            setCheckoutStep(2)
                          }}
                          className="rounded-full bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-black px-6 py-2.5 text-xs transition-all cursor-pointer font-outfit"
                        >
            ...
            {/* STEP 2: FULFILLMENT MODE & ADDRESS */}
            <div className={`rounded-3xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm space-y-4 font-outfit transition-all ${checkoutStep !== 2 ? 'opacity-85' : 'ring-2 ring-orange-500'}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-orange-500 text-white font-black text-xs font-outfit">
                    <span className="font-outfit text-sm">2</span>
                  </div>
                  <div>
                    <h3 className="font-outfit text-base sm:text-lg font-black text-slate-900">Fulfillment & Delivery</h3>
                    {checkoutStep > 2 && (
                      <p className="text-xs text-emerald-600 font-bold">
                        {orderMode === 'Takeaway' ? 'Self Pickup at Kaveri Layout' : `Deliver to ${activeAddr.slice(0, 35)}...`} ✓
                      </p>
                    )}
                  </div>
                </div>
              ...
              {checkoutStep === 2 && (
                <div className="space-y-4 animate-in fade-in duration-200">
                  ...
                  {orderMode === 'Delivery' && (
                    <div className="space-y-3 font-outfit animate-in fade-in">
                      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-2 text-xs">
                        <div className="flex items-center justify-between">
                          ...
                          <button onClick={onOpenPermissions} className="text-orange-600 font-bold hover:underline cursor-pointer">Change Location</button>
                        </div>
                  ...
                  {orderMode === 'Takeaway' && (
                    <div className="rounded-2xl border border-orange-500/30 bg-orange-500/5 p-4 text-xs space-y-2.5 font-outfit animate-in fade-in">
                      <div>
                        ...
                        <p className="text-slate-500 text-[11px] mt-0.5">Your meal will be packed piping hot in 15 mins. No delivery fee charged!</p>
                      </div>
                      <div>
                        <a 
                          href="https://www.google.com/maps?sca_esv=9035e449288dd937&sxsrf=APpeQntqg-HDxuYyVzBGTyUz6Kj2OFrIsQ:1787238219933&gs_lp=Egxnd3Mtd2l6LXNlcnAiIGJvYnMgc2F0ZWxsaXRlIGtpdGNoZW4gYmFuZ2Fsb3JlMgQQIxgnMgoQABiABBiKBRhDMggQABiABBixAzIQEC4YgAQYigUYQxjHARjRAzIFEAAYgAQyChAAGIAEGIoFGEMyBRAAGIAEMgcQABiABBgKMgsQABiABBixAxiDATIFEAAYgARI3BNQ_&um=1&ie=UTF-8&fb=1&gl=in&sa=X&geocode=KYHKdZuqE647MbNdGsuyg8GT&daddr=1067,+8th+Main+Rd,+Kaveri+Layout,+Marathahalli+Village,+Marathahalli,+Bengaluru,+Karnataka+560037"
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold px-4 py-2 text-xs transition-all shadow-sm cursor-pointer"
                        >
                          📍 Get Directions
                        </a>
                      </div>
                  ...
                  <div className="pt-2 flex justify-end">
                    <button
                      ...
                      className="rounded-full bg-orange-500 hover:bg-orange-600 text-white font-black px-6 py-2.5 text-xs transition-all cursor-pointer font-outfit"
                    >
                      Continue to Payment →
                    </button>
                  </div>
            ...
            <div className={`rounded-3xl border border-slate-200 bg-white p-5 sm:p-6 shadow-sm space-y-4 font-outfit transition-all ${checkoutStep !== 3 ? 'opacity-85' : 'ring-2 ring-orange-500'}`}>
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-600 text-white font-black text-xs font-outfit">
                  <span className="font-outfit text-sm">3</span>
                </div>
                <div>
                  <h3 className="font-outfit text-base sm:text-lg font-black text-slate-900">Payment Option</h3>
                  <p className="text-xs text-slate-500">Choose between dynamic UPI QR or Direct App Link</p>
                </div>
              </div>

              {checkoutStep === 3 && (
                <div className="space-y-4 animate-in fade-in duration-200 font-outfit">
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      type="button"
                      onClick={() => setPaymentMethod('qr')}
                      className={`flex flex-col items-center justify-center gap-1 rounded-xl p-3 border transition-all cursor-pointer ${
                        paymentMethod === 'qr'
                          ? 'border-orange-500 bg-orange-500/5 shadow-xs font-black'
                          : 'border-slate-200 bg-white hover:bg-slate-50 font-bold'
                      }`}
                    >
                      <span className="text-xl">📱</span>
                      <span className="text-[11px] text-slate-900">Dynamic QR Code</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <span className="text-xl">⚡</span>
                      <span className="text-[11px] text-slate-900">Direct UPI Link</span>
                    </button>
                  </div>

                  {paymentMethod === 'qr' && (
                    <div className="flex flex-col items-center justify-center bg-white border border-orange-200/80 rounded-2xl p-6 shadow-sm space-y-4 animate-in fade-in">
                      <span className="rounded-full bg-orange-500/10 text-orange-600 border border-orange-500/30 px-3 py-1 text-[9px] font-black uppercase tracking-wider">
                        Scan to Pay via GPay / PhonePe / Paytm
                      </span>
                      
                      <div className="relative size-52 bg-white p-3 rounded-2xl shadow-md border-4 border-orange-500 flex items-center justify-center overflow-hidden">
                        {qrDataUrl ? (
                          <>
                            <img 
                              src={qrDataUrl} 
                            ...
                            />
                            {paymentStatus === 'paid' && (
                              <div className="absolute inset-0 bg-white/90 flex flex-col items-center justify-center text-center p-3 z-10">
                                <span className="text-emerald-500 text-3xl font-black">✓</span>
                                <p className="text-xs font-bold text-emerald-800 mt-1">Paid Successfully</p>
                              </div>
                            )}
                          </>
                        ) : (
                          <div className="animate-pulse flex flex-col items-center justify-center text-slate-300 gap-1">
                            <span className="text-3xl animate-bounce">⚡</span>
                            <span className="font-bold text-[10px] uppercase">Generating QR...</span>
                          </div>
                        )}
                      </div>

                      <div className="text-center space-y-1">
                        <p className="text-xs text-slate-500 font-bold">Payable Amount:</p>
                        <p className="text-2xl font-black text-orange-600">{formatPrice(total)}</p>
                        <p className="text-[10px] text-slate-400 font-mono font-bold">Order Ref: {qrOrderId || 'Generating...'}</p>
                      </div>
                    </div>
                  )}

                  {paymentMethod === 'intent' && (
                    <div className="flex flex-col items-center justify-center bg-white border border-orange-200/80 rounded-2xl p-5 shadow-sm space-y-4 text-center animate-in fade-in w-full">
                      <span className="rounded-full bg-emerald-500/10 text-emerald-600 border border-emerald-500/30 px-3 py-1 text-[9px] font-black uppercase tracking-wider">
                        Pay directly using your UPI apps
                      </span>
                      
                      <div className="space-y-1.5 text-center">
                        <p className="text-[11px] text-slate-500 font-bold">Payable Amount:</p>
                        <p className="text-2xl font-black text-orange-600">{formatPrice(total)}</p>
                      </div>

                      {upiLinks ? (
                        <div className="w-full grid grid-cols-2 gap-2 text-xs font-outfit">
                          <a 
                            href={upiLinks.gpay}
                            onClick={() => {
                              setTimeout(() => {
                                alert("If Google Pay didn't open, try the generic 'Pay via UPI' button instead.");
                              }, 2500);
                            }}
                            className="flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 p-3 font-bold text-slate-800 shadow-2xs cursor-pointer transition-all"
                          >
                            🔵 Google Pay
                          </a>
                          <a 
                            ...
                            onClick={() => {
                              setTimeout(() => {
                                alert("If PhonePe didn't open, try the generic 'Pay via UPI' button instead.");
                              }, 2500);
                            }}
                            className="flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 p-3 font-bold text-slate-800 shadow-2xs cursor-pointer transition-all"
                          >
                          ...
                          <a 
                            ...
                            onClick={() => {
                              setTimeout(() => {
                                alert("If Paytm didn't open, try the generic 'Pay via UPI' button instead.");
                              }, 2500);
                            }}
                            className="flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 p-3 font-bold text-slate-800 shadow-2xs cursor-pointer transition-all"
                          >
                          ...
                          <a 
                            ...
                            className="flex items-center justify-center gap-1.5 rounded-xl bg-slate-900 text-white hover:bg-slate-800 p-3 font-black shadow-sm cursor-pointer transition-all"
                          >
                            ⚡ Pay via UPI
                          </a>
                        </div>
                      ) : (
                        <p className="text-xs text-slate-400 animate-pulse uppercase font-black">Initializing links...</p>
                      )}

                      <div className="text-[9px] text-slate-400 font-bold">
                        Note: Deep links work best on Android devices with installed apps.
                      </div>
                    </div>
                  )}
                  ...
                  <button
                    ...
                    className="w-full rounded-2xl bg-gradient-to-r from-orange-500 via-orange-500 to-amber-500 py-4 text-sm font-black text-white shadow-xl hover:scale-[1.01] active:scale-[0.99] transition-all cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {isPlacingOrder ? (
...
export function RestaurantApp({ onAccount }: { onAccount?: () => void }) {
  ...
  const [deliveryCoordinates, setDeliveryCoordinates] = useState({ lat: KITCHEN_LAT, lng: KITCHEN_LNG })
  ...
  useEffect(() => {
    const channel = supabase
      .channel('website-order-status')
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders' }, (payload) => {
        const updatedOrder = payload.new as { order_id?: string; status?: string }
        if (!updatedOrder.order_id || !updatedOrder.status) return
        useAppStore.setState((state) => ({
          orders: state.orders.map((order) => order.id === updatedOrder.order_id ? { ...order, status: updatedOrder.status! } : order),
        }))
      })
      .subscribe()
  ...
  const handleOpenTracking = async (orderId: string, row?: any) => {
    const { data } = row ? { data: row } : await supabase.from('orders').select('*').eq('order_id', orderId).single()
    if (data) {
      const trackedOrder = orderFromSupabaseRow(data)
      useAppStore.setState((state) => ({ orders: [trackedOrder, ...state.orders.filter((order) => order.id !== orderId)] }))
    }
    ...
    <div className="min-h-screen bg-[#FAF8F5] text-foreground flex flex-col justify-between font-sans">
      <div>
        ...
        <FormattedHeader 
          ...
          onTrackOrders={() => setPhoneLookupOpen(true)}
          onOpenPermissions={() => setAddressModalOpen(true)} 
      ...
      <AddressSelectionModal 
        ...
        onSelectAddress={(addr, dist, lat, lng) => {
          ...
          if (lat !== undefined && lng !== undefined) setDeliveryCoordinates({ lat, lng })
      ...
      />
      <PhoneOrderLookup open={phoneLookupOpen} onClose={() => setPhoneLookupOpen(false)} onTrack={handleOpenTracking} />
    </div>
  )
}
