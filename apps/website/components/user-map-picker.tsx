...
export function UserMapPicker({ 
  ...
  const fetchAddress = async (lat: number, lng: number) => {
    ...
    try {
      ...
      if (data && data.address) {
        ...
        const doorNumber = addr.house_number || addr.building || 'Flat/Door No.'
        const road = addr.road || addr.street || addr.footway || 'Main Road'
        const area = addr.suburb || addr.neighbourhood || addr.residential || addr.commercial || addr.village || addr.city_district || addr.subdistrict || 'Marathahalli'
        const pincode = addr.postcode || '560037'
        
        finalAddr = `${doorNumber}, ${road}, ${area}, Bengaluru - ${pincode}`
  ...
  useEffect(() => {
    ...
    // Try to get user coordinates to center map dynamically on load instead of hardcoding
      ...
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          try {
            ...
            console.warn('Map setView error avoided:', err)
          }
          
          setCurrentLat(uLat)
          setCurrentLng(uLng)
          const newDist = calculateDistanceKm(KITCHEN_LOCATION[0], KITCHEN_LOCATION[1], uLat, uLng)
          setDistanceKm(newDist)
          userMarker.setIcon(createUserPinIcon(newDist <= 3.0))
          fetchAddress(uLat, uLng)
        },
        () => {},
