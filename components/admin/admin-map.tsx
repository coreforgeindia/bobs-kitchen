...
function createCustomIcon(type: 'kitchen' | 'delivery' | 'delivered', status?: string) {
  let bgColor = '#ea580c' // Orange default
  let svgPath = `<path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/>`

  if (type === 'kitchen') {
    bgColor = '#0284c7' // Cyan glowing kitchen
    ...
    svgPath = `<path d="M5.5 17a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Z"/><path d="M18.5 17a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Z"/><path d="M15 6h2l3 7v4h-2"/><path d="M5.5 17H3V6h9v11"/>`
  ...
  const html = `
    <div style="
      ...
      cursor: pointer;
    ">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ffffff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">${svgPath}</svg>
      <div style="
        position: absolute;
        bottom: -6px;
        left: 50%;
...
export function AdminMap() {
  ...
  useEffect(() => {
    ...
    // CartoDB Dark Matter tile layer for premium aesthetic
      ...
      marker.bindPopup(`
        <div style="font-family: system-ui, sans-serif; width: 230px; padding: 4px; color: #18181b;">
          <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #e4e4e7; padding-bottom: 6px;">
            ...
            <span style="background: #ffedd5; color: #c2410c; padding: 2px 6px; border-radius: 4px; font-size: 10px; font-weight: bold;">
              ${order.mode}
            </span>
          </div>
  ...
  const resetView = () => {
    ...
    if (mapRef.current) {
      mapRef.current.flyTo(KITCHEN_LOCATION, 13, { duration: 1 })
    }
  }
  ...
  return (
    <div className="relative h-[420px] w-full overflow-hidden rounded-3xl border border-slate-200 bg-slate-900 shadow-xl">
      {/* Top Filter Bar */}
      <div className="absolute top-4 left-4 right-4 z-20 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-zinc-900/90 px-4 py-3 backdrop-blur-xl border border-white/10 shadow-lg">
        ...
        <div className="flex items-center gap-2">
          {['All', 'Order Received', 'Preparing', 'Out For Delivery', 'Delivered'].map((status) => (
            <button
              key={status}
